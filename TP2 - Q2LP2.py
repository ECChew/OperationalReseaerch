...
p1 += 1 * x1 + 1 * x2 - 1 * x3 >= 0, "Third constraint"
...
#Initialize variables
p2 = pulp.LpProblem("First problem", pulp.LpMinimize)
#Initialize variables, Use LpContinuous for R, LpInteger
...
p2 += 2 * x1 + 7 * x2 + 2 * x3
#Add constraints
p2 += 1 * x1 + 4 * x2 + 1 * x3 >= 10.0, "First constraint"
p2 += 4 * x1 + 2 * x2 + 2 * x3 >= 13, "Second constraint"
p2 += 1 * x1 + 1 * x2 - 1 * x3 >= 0, "Third constraint"
#Write LP into a .lp file
