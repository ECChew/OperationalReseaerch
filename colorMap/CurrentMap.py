...
def getResultBinary(vote, pop):
    ...
    for i in range(len(vote)):
        print((pop[i]))
        s += float(vote[i][0]) * float(pop[i][0])

    print(s)
    return s
with open('D:\Pycharm\OperationalReseaerch\DataElection.csv', newline='') as f:
    reader = csv.reader(f)
    pop = list(reader)
...
with open('D:\Pycharm\OperationalReseaerch\RealElectors.csv', newline='') as f:
    reader = csv.reader(f)
    realAlpha = list(reader)

print(float(realAlpha[0][0]))
newstateID = [i[0] for i in alphalab]
print(newstateID)
newalphares = [float(realAlpha[i][0])*1e6/float(pop[i][0]) for i in range(len(realAlpha))]
zipbObj = zip(newstateID, newalphares)
dictOfWords = dict(zipbObj)
print(dictOfWords)
s = getResultBinary(binaryvote, realAlpha)
print(N/2)
if (s < N/2) :
    print("Winner : Trump")
elif (s > N/2):
...
ax.set_title('Number of electors per million of inhabitants - Current state')
...
cb = ColorbarBase(ax_c,cmap=cmap,norm=norm,orientation='vertical',
                  label=r'[Electoral college members / M inhabitants]')
